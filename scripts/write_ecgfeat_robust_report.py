#!/usr/bin/env python3
"""Render the Chinese implementation/evidence report from measured artifacts."""
import json
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
folder=ROOT/"ecgfeat_robust_20260922"
x=json.loads((folder/"analysis.json").read_text())
runtime=json.loads((folder/"profile_runtime.json").read_text())
...
def but(variant,lead="II",wave="P",tol=150,cohort="all"):
    return next(r for r in x["but"][cohort] if (r["variant"],r["lead"],r["wave"],r["tolerance_ms"])==(variant,lead,wave,tol))


def gudb(method,stream,tol=75,activity=None):
    rows=x["gudb"]["overall"] if activity is None else x["gudb"]["metrics"]
    return next(r for r in rows if (r["method"],r["lead"],r["protocol"])==(method,stream,f"peak_{tol}ms")
                and (activity is None or r["activity"]==activity))


but_table=table(["配置","P F1 / 50 ms (%)","P F1 / 150 ms (%)","P TP / FP / FN (150 ms)","QRS F1 / 75 ms (%)"],
    [[v,percent(but(v,tol=50)["f1"]),percent(but(v)["f1"]),
      " / ".join(str(but(v)[k]) for k in ("tp","fp","fn")),percent(but(v,wave="QRS",tol=75)["f1"])]
     for v in ("default","robust_p","limited","limited_p","limited_qrs","limited_robust")])
nstdb_table=table(["SNR (dB)","原 QRS F1 (%)","改进 QRS F1 (%)","改进 TP / FP / FN"],
    [[snr,*[percent(next(r for r in x["nstdb"] if r["method"]==v and r["snr"]==snr and r["phase"]=="noisy")["f1"])
                for v in ("default","qrs_adaptive_consensus")],
      " / ".join(str(next(r for r in x["nstdb"] if r["method"]=="qrs_adaptive_consensus" and r["snr"]==snr and r["phase"]=="noisy")[k]) for k in ("tp","fp","fn"))]
     for snr in (24,18,12,6,0,-6)])
gudb_table=table(["设备流","原 F1 / 75 ms (%)","改进 F1 / 75 ms (%)","原 → 改进 F1 / 50 ms (%)","改进 TP / FP / FN"],
    [[stream,percent(gudb("default",stream)["f1"]),percent(gudb("adaptive",stream)["f1"]),
      percent(gudb("default",stream,50)["f1"])+" → "+percent(gudb("adaptive",stream,50)["f1"]),
      " / ".join(str(gudb("adaptive",stream)[k]) for k in ("tp","fp","fn"))]
     for stream in ("cables","chest_strap")])
gudb_activity=table(["活动","双导联原 F1 (%)","双导联改进 F1 (%)"],
    [[activity,percent(gudb("default","cables",activity=activity)["f1"]),percent(gudb("adaptive","cables",activity=activity)["f1"])]
     for activity in ("sitting","maths","walking","hand_bike","jogging")])
isp_rows=[r for r in x["isp"] if r["split"]=="test" and r["wave"]=="T" and r["lead"]=="consensus_median"]
isp_table=table(["T 配置","匹配数","F1 (%)","Ton MAE (ms)","Toff MAE (ms)"],
    [[r["method"],r["tp"],percent(r["f1"]),f'{r["onset_mae_ms"]:.3f}',f'{r["offset_mae_ms"]:.3f}'] for r in isp_rows])
ludb_table=table(["配置","P-on MAE / 有效数","P-off MAE / 有效数","T-off MAE / 有效数","QRS 匹配数"],
    [[v,*[f'{r[k]["mae"]:.3f} / {r[k]["n"]}' for k in ("p_on_err","p_off_err","t_off_err")],r["coverage"]["matched_qrs"]] for v,r in x["ludb"].items()])
runtime_table=table(["数据 / 记录","配置","信号时长(s)","提取中位耗时(s)","检测搏数"],
    [[r["dataset"]+" / "+r["record"],r["variant"],r["signal_seconds"],f'{r["median_seconds"]:.3f}',r["beats"]] for r in runtime["records"]])
accepted=but("limited_robust",lead="accepted_native")
atrial=but("limited_robust",lead="atrial_events")
no_p_table=table(["记录","旧 native P FP","改进 native P FP","改进 accepted P FP"],
    [[name,*[next(r for r in x["but_no_p"] if r["record"]==name and r["method"]==v and r["lead"]==lead)["fp"]
              for v,lead in [("default","II"),("limited_robust","II"),("limited_robust","accepted_native")]]]
     for name in ("08","48")])
ci_table=table(["任务","F1 提升(百分点)","95% 按受试者 bootstrap 区间(百分点)"],
    [[r["stream"]+" / "+str(r["tolerance_ms"])+" ms",f'{100*r["pooled_f1_difference"]:.3f}',
      f'[{100*r["ci95"][0]:.3f}, {100*r["ci95"][1]:.3f}]'] for r in x["gudb_paired_f1"]])

report=f'''# ECG 传统提取算法改进与完整验证（2026-09-22）
...
{accepted['tp']}/{accepted['fp']}/{accepted['fn']}，precision={percent(accepted['precision'])}%，recall={percent(accepted['sensitivity'])}%。
原始心房事件流的 TP/FP/FN 为 {atrial['tp']}/{atrial['fp']}/{atrial['fn']}，precision={percent(atrial['precision'])}%；
这个候选流不能直接作为正式 P 波清单。
...
'''
(ROOT/"docs/ecgfeat_robust_20260922.md").write_text(report)
print("docs/ecgfeat_robust_20260922.md")
