...
3. **NPZ sidecar locator and integrity fields.** The sidecar is already the preferred dense-matrix transport, but the brief does not settle whether its JSON reference is relative-path-only, content-addressed, or supports external URIs. The public query API should stay unchanged; the schema must settle locator and checksum rules before sidecar loading is implemented.
