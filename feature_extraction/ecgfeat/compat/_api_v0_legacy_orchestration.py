"""Preserved pre-decomposition ``ECGFeatureExtractor`` orchestration (rollback path).

This is ``feature_extraction/ecgfeat/api.py`` exactly as it was at commit e7c26f7,
before the Phase 3 decomposition into pipeline stages and policies: the full
``extract`` body and all 49 private helpers, with only the relative import levels
adjusted for this module's location (one package deeper).  It is not maintained
code; it exists so that the staged pipeline can be rolled back without a code
change (docs/library_design/05_migration_plan.md, Phase 3 "Rollback" and section F).

Route extraction here by setting ``ECGFEAT_LEGACY_ORCHESTRATION=1``; see
``ecgfeat.compat.api_v0.extract_legacy_features``.  Do not delete this module in the
same release that first makes the staged pipeline authoritative.
"""
