...
def av_dissociation_corroborated(rule_summary) -> bool:
    ...
    Measurement-availability copy of the predicate the rule engine also uses
    (``ecginterpret.clinical_rules.rhythm``); a parity test keeps them equal.
    """
...
def disorganized_atrial_activity(summary, *, threshold: float) -> Optional[float]:
    ...
    Measurement-availability copy of the rule-engine predicate; None when the
    ratio is organized enough or was not measured.
    """
...
def build_measurement_availability(
    ...
    if "af_afl_indeterminate" in reason_set and disorganized_atrial_activity(
        af_afl_summary,
        threshold=ENGINE_THRESHOLDS.rhythm.organized_p_ratio_min_for_morphology,
    ) is not None:
